...
class _NoteLandingState extends State<NoteLanding> {
  ...
  void _onReorder(int oldIndex, int newIndex) {
    ...
    if (isFirst) {
      ...
      num pNodeSequence =
          _items[oldIndex < newIndex ? newIdx : newIdx - 1].sequence;
      num aNodeSequence =
          _items[oldIndex > newIndex ? newIdx : newIdx + 1].sequence;
                  ...
                  onChanged: (v) async {
                    if (v != null && v.isNotEmpty) {
                      _items = await db.getNotes({
                        ' where ( title like ? ': '%$v%',
                        ' or content like ?) ': '%$v%',
                        switcherProvider.isHiddenDone() ? ' and isDone=? ' : '':
                            switcherProvider.isHiddenDone() ? 0 : null
                      });
                      setState(() {});
                    } else {
    ...
    return BottomAppBar(
        ...
        child: Row(children: <Widget>[
          ...
          IconButton(
              ...
              onPressed: () {
                if (this._currentNote != null && this._currentNote.isDone) {
                  return showDialog<void>(
                      context: context,
                      builder: (context) {
                        return AlertDialog(
  ...
  List<CheckboxListTile> _buildItemList(ThemeData theme) {
    List<CheckboxListTile> _listTiles = _items.map((item) {
      String __key = '${item.id}cblt';
      _ctrls.putIfAbsent(
          __key, () => TextEditingController(text: item.content));
